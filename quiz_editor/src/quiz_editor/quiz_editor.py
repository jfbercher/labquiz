            ...
            with cb3:
                ...
                if not is_template:
                    if q_data.get('type') == 'mcq':
                        ...
                        p['expected'] = st.number_input(_("Expected value"), value=p.get('expected', 0.0), key=f"exp_{q_id}_{i}")
                else:
...
def main():
    ...
    st.set_page_config(layout="wide", page_title=_("YAML Editor - {FILE_PATH}").format(FILE_PATH=FILE_PATH),  
                    page_icon=":page_with_curl:") #"quiz_editor/src/quiz_editor/1F4C3.png") #📃")
    apply_custom_styles()
